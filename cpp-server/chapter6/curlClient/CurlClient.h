...
public:
     ...
     * @param response 如果请求成功，则存储HTTP请求结果（注意：response在函数调用中是追加模式，也就是说如果上一次response的值不清空，则调用这个函数时会追加，而不是覆盖）
     * @param autoRedirect 请求得到HTTP 3xx的状态码是否自动重定向至新的URL
     * @param reserveHeaders 请求的结果中（存储于response），是否保留HTTP头信息
     * @param connTimeout 连接超时时间，单位为秒（对于某些HTTP URI资源不好的，若总是返回超时，则可以将该参数设置得大一点）
     * @param readTimeout 读取数据超时时间，单位为秒（对于某些HTTP URI资源不好的，若总是返回超时，则可以将该参数设置得大一点）
     */
     ...
     * @param response 如果请求成功，则存储HTTP请求的结果（注意：response在函数调用中是追加模式，也就是说如果上一次response的值不清空，则调用这个函数时会追加，而不是覆盖）
     * @param autoRedirect 请求得到HTTP 3xx的状态码是否自动重定向至新的URL
     * @param bReserveHeaders 请求的结果中（存储于response）是否保留HTTP头信息
     * @param connTimeout 连接超时时间，单位为秒（对于某些HTTP URI资源不好的，若总是返回超时，则可以将该参数设置得大一点）
     * @param readTimeout 读取数据超时时间，单位为秒（对于某些HTTP URI资源不好的，若总是返回超时，则可以将该参数设置得大一点）
     */
